<template>
    <div class="container">
      ...
      <div class="input-container">
        ...
        <input type="text" v-model="targetName" class="input-field" placeholder="Enter Target Name">
        <button @click="fetchData" class="btn">Confirm & Upload</button>
      </div>
  ...
  const id = ref("");
  const targetName = ref(""); // Define targetName variable
  const headers = ref([]);
  const items = ref([]);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://localhost:8000/datapi/precessing_data/${id.value}?target_name=${targetName.value}`);
      if (!response.ok) {
