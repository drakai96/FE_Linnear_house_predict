...
export default {
  setup() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:8000/visualization/datapoint/1');
    ...
    const options = computed(() => {
      return {
        type: "column", // Change type to "column" for a bar chart
        animationEnabled: true,
        exportEnabled: true,
        theme: "light2",
        title: {
          text: "Number NeiborHoods"
        },
        axisX: {
          valueFormatString: "Type",
          labelTextAlign: "center",
          labelAngle: 0
        },
        axisY: {
          title: "Amount NeiborHoods",
          valueFormatString: "##"
        },
        data: [{
          type: "column", // Change type to "column" for a bar chart
